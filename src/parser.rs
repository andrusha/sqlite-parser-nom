...
pub fn database(i: &[u8]) -> IResult<&[u8], Database> {
    ...
    let (i, root_page) = map_parser(
        take(page_size - HEADER_SIZE),
        page::<HEADER_SIZE>)(i)?;

    let (i, mut pages) = complete(many0(
        map_parser(
            take(page_size),
            page::<0>)))(i)?;
...
fn db_header(i: &[u8]) -> IResult<&[u8], DbHeader> {
    ...
    let (i, (max_payload_fraction, min_payload_fraction, leaf_payload_fraction)) = (be_u8, be_u8, be_u8).parse(i)?;
...
fn page<const OFFSET: usize>(i: &[u8]) -> IResult<&[u8], Page> {
    alt((
        map(interior_index_b_tree_page::<OFFSET>, |p| Page::InteriorIndexPage(p)),
        map(leaf_index_b_tree_page::<OFFSET>, |p| Page::LeafIndexPage(p)),
        map(interior_table_b_tree_page::<OFFSET>, |p| Page::InteriorTablePage(p)),
...
fn column_values<'a, 'b>(i: &'a [u8], serial_types: &'b [SerialType]) -> IResult<&'a [u8], Vec<Option<Payload>>> {
    let mut i: &[u8] = i;
